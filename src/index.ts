...
export interface WiredElement extends ui.Element {
  appliedEvents?: any[];
}
export type Component = ui.Component<any, any>;

export type DomTypes = Element | Element[] | Text;
export type Domish = DomTypes | DomTypes[];
...
export function render(o: any): Domish {
  if (o === undefined) {
    ...
    return renderComponent(o);
  } else if (o instanceof ui.Element) {
    return renderElement(o);
  } else {
    return;
  }
}

function applyEvents(e: WiredElement, dom: HTMLElement) {
...
export function renderElement(e: WiredElement): HTMLElement {
  if (!e.dom) {
    e.appliedEvents = [];
  }
...
export function syncProps(e: WiredElement, dom?: Element): void {
  if (!dom) {
    dom = e.dom;
  }
